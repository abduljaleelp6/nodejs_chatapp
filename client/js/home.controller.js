...
app.controller('homeController', function($scope, $routeParams, $location, appService) {
    ...
    appService.httpCall({
        ...
        .then((response) => {
            $scope.data.business_logo = response.business_logo;
    ...
    $scope.selectFriendToChat = (friendId) => {
        ...
        // will take data from selected chalist 
        $scope.data.selectedFriendName = friendData[0]['business_name'];
        $scope.data.selectedFriendImg = friendData[0]['business_logo'];
